import { baseDeConocimiento } from './database/conocimiento.js';
    ...
    async function initializeAppLogic() {
        ...
        if (saveProfileBtn) {
            saveProfileBtn.addEventListener('click', () => {
                alert('Funcionalidad "Guardar Cambios" no implementada todavía.');
            });
        }
        ...
        if (changePasswordBtn) {
            changePasswordBtn.addEventListener('click', () => {
                alert('Funcionalidad "Cambiar Contraseña" no implementada todavía.');
            });
        }
        ...
        if (deleteAccountBtn) {
            deleteAccountBtn.addEventListener('click', () => {
                if (confirm('¿Estás seguro de que quieres eliminar tu cuenta? Esta acción es permanente.')) {
                    alert('Funcionalidad "Eliminar Cuenta" no implementada todavía.');
                }
            });
        }
        ...
        async function sendMessage() {
            ...
            const messageData = { type: 'user', text: texto, pictogramas, timestamp: firebase.firestore.FieldValue.serverTimestamp(), contentType: 'text' };
        ...
        function handleFileUpload(file) {
            if (!file) return;
            console.log("Mostrando vista previa local para el archivo:", file.name);
            const reader = new FileReader();
            reader.onload = function (event) {
                let localMessage = { type: 'user', timestamp: new Date() };
                if (file.type.startsWith('image/')) {
                    localMessage.fileUrl = event.target.result;
                    localMessage.fileName = file.name;
                    localMessage.contentType = 'image';
                } else {
                    localMessage.text = `(Vista previa) Archivo adjunto: ${file.name}`;
                    localMessage.contentType = 'text';
                }
                renderMessage(localMessage, false);
            };
            if (file.type.startsWith('image/')) {
        ...
        function renderMessage(msg, animate = false) {
            ...
            const botAvatar = `<img src="images/Agente.webp" alt="Bot" class="message-avatar">`;
                ...
                try {
                    ...
                    mediaRecorder.onstop = () => {
                        ...
                        reader.onload = (event) => {
                            ...
                            const localMessage = { type: 'user', contentType: 'audio', fileUrl: audioDataUrl, timestamp: new Date() };
                            renderMessage(localMessage, false);
                        };
                        audioChunks = [];
        ...
        function obtenerRespuesta(preguntaUsuario) {
            ...
            const saludos = ['hola', 'ola', 'buenos dias', 'buenas tardes', 'buenas noches', 'hey', 'que tal'];

            if (saludos.includes(preguntaNormalizada)) {
                return "Hola, soy K-I-B-O, tu asistente. ¿En qué te puedo ayudar?";
            }

            const tokensUsuario = new Set(preguntaNormalizada.split(' '));
            let mejorCoincidencia = null;
            let maxPuntuacion = 0;

            baseDeConocimiento.forEach(item => {
                if (!item.pregunta_tokenizada || !Array.isArray(item.pregunta_tokenizada)) {
                    return;
                }
                let puntuacionActual = 0;
                const tokensPreguntaDB = new Set(item.pregunta_tokenizada.map(t => t.toLowerCase().replace(/[.,!?;¿¡'"]/g, '')));
                tokensUsuario.forEach(tokenUsuario => {
                    if (tokensPreguntaDB.has(tokenUsuario)) {
                        puntuacionActual++;
                    }
                });
            ...
            if (mejorCoincidencia && maxPuntuacion >= 2) {
        ...
        async function buscarPictogramas(texto) { const palabras = texto.toLowerCase().replace(/[.,!?;¿¡]/g, '').split(' '); const pictogramasUnicos = new Map(); for (const palabra of palabras) { if (palabra.length > 2) { try { const response = await fetch(`https://api.arasaac.org/v1/pictograms/es/search/${palabra}`); if (response.ok) { const resultados = await response.json(); if (resultados.length > 0 && !pictogramasUnicos.has(resultados[0]._id)) { pictogramasUnicos.set(resultados[0]._id, `https://api.arasaac.org/v1/pictograms/${resultados[0]._id}`); } } } catch (error) { console.error("Error buscando pictograma:", error); } } } return Array.from(pictogramasUnicos.values()); }
