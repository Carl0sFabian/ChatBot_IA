document.addEventListener('DOMContentLoaded', async () => {
    ...
    async function sendMessage() {
        ...
        const messageData = {
            type: 'user', text: texto, pictogramas,
    ...
    function simulateBotResponse(userText) {
        setTimeout(async () => {
            ...
            const botMessage = {
                type: 'bot', text: botResponse, pictogramas,
                timestamp: firebase.firestore.FieldValue.serverTimestamp(),
                contentType: 'text'
            };
            if (currentChatId) {
    ...
    function renderMessage(msg) {
        ...
        const userAvatar = `<img src="images/VS.png" alt="User" class="message-avatar">`;
        ...
        confirmChatBtn.addEventListener('click', () => {
            ...
            if (newChatName) {
                db.collection('chats').add({
                    name: newChatName, avatar: 'images/Logo.jpeg',
                    createdAt: firebase.firestore.FieldValue.serverTimestamp(),
                    userMessageCount: 0, unlockedTrophies: []
                }).then(() => {
    ...
    function updateDashboard(chats) {
        ...
        let totalTrophies = 0;
        chats.forEach(chat => { totalTrophies += (chat.unlockedTrophies || []).length; });
    ...
    function updateClock() {
        ...
        const now = new Date();
        dateTimeCardDate.textContent = now.toLocaleDateString('es-ES', { weekday: 'long', year: 'numeric', month: 'long', day: 'numeric' });
    ...
    async function checkAndUnlockTrophies(chatId) {
        ...
        if (newTrophiesToUnlock.length > 0) {
            await chatRef.update({ unlockedTrophies: firebase.firestore.FieldValue.arrayUnion(...newTrophiesToUnlock) });
        }
        updateTrophyPanel({ ...chatData, id: chatId, userMessageCount: currentCount, unlockedTrophies: [...currentTrophies, ...newTrophiesToUnlock] });
    }
    ...
    function updateTrophyPanel(chat) {
        ...
        let nextGoalCount = Infinity, nextGoalInfo = null;
    ...
    async function cargarBaseDeConocimiento() {
        ...
        try {
            ...
            const idxRespuesta = cabeceras.indexOf('respuesta'), idxPregunta = cabeceras.indexOf('pregunta');
            if (idxPregunta === -1 || idxRespuesta === -1) return;
            baseDeConocimiento = filas.map(fila => {
                const valores = fila.split(',');
                return { pregunta: valores[idxPregunta] || '', respuesta: valores[idxRespuesta] || '' };
            });
        } catch (error) { console.error("Error al cargar la base de conocimiento:", error); }
    }
    ...
    async function buscarPictogramas(texto) {
        ...
        for (const palabra of palabras) {
            if (palabra.length > 2) {
                ...
                } catch (error) { console.error("Error buscando pictograma:", error); }
            }
        }
